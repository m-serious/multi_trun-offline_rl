...
def run_ppo(config) -> None:
    ...
    if is_offline_mode:
        ...
        if not config.data.get("offline_train_files"):
            raise ValueError("Offline mode is enabled, but 'config.data.offline_train_files' is not set.")
        if not config.data.get("offline_val_files"):
            print("WARN: 'config.data.offline_val_files' is not set for offline mode. Validation might not work as expected.")
    else:
        print("INFO: Running in ONLINE RL mode.")
...
class TaskRunner:
    def run(self, config):
        ...
        trainer = RayPPOTrainer(
            ...
            collate_fn=collate_fn, # This collate_fn will need to handle the output of OfflineRLDataset
...
def create_offline_rl_dataset(data_paths, data_config, tokenizer, processor, is_multi_turn=False):
    ...
    # For example, in verl.utils.dataset.rl_dataset
    try:
