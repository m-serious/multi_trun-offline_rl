...
class RayPPOTrainer:
    ...
    def _validate_config(self):
        ...
        if config.actor_rollout_ref.rollout.multi_turn.enable:
            if not self.is_offline_mode: # Original check for online
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler):
        ...
        from verl.trainer.main_ppo import create_rl_dataset, create_rl_sampler, create_offline_rl_dataset # ensure create_offline_rl_dataset is imported
        ...
        self.train_dataloader = StatefulDataLoader(
            ...
            # For offline, train_batch_size is the number of trajectories/transitions per batch
        ...
        if self.val_dataset:
            ...
            if self.is_offline_mode and val_batch_size is None: # Default for offline validation
                val_batch_size = len(self.val_dataset) if self.val_dataset else 1 # Process all val data
            elif val_batch_size is None: # Default for online validation
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # The structure of batch_dict from OfflineRLDataset needs to be compatible
                # with DataProto or you adapt how DataProto is constructed here.
                # Assuming collate_fn produces a dict that DataProto.from_single_dict can handle
                # and it contains: "input_ids", "attention_mask", "responses" (actions from dataset),
                # "rewards" (from dataset), "loss_mask" (from dataset for multi-turn).
                ...
                with _timer("step", timing_raw):
                    if not self.is_offline_mode: # ONLINE Mode: Generate sequences
                        # This part is for online RL
                        # Pop keys for generation
                        batch_keys_to_pop = ["input_ids", "attention_mask", "position_ids"]
                        non_tensor_batch_keys_to_pop = ["raw_prompt_ids"]
                        # ... (pop other keys like multi_modal_inputs, raw_prompt, tools_kwargs) ...
                        gen_batch = batch.pop(
                            batch_keys=batch_keys_to_pop,
                            non_tensor_batch_keys=non_tensor_batch_keys_to_pop,
                        )
                        ...
                        # For REMAX, baseline generation (if kept for offline, would need offline actions for baseline)
                        if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                             # This part needs careful adaptation for offline if REMAX is used.
                             # It might involve loading a separate set of baseline actions from data.
                            pass # Placeholder - REMAX baseline gen needs offline adaptation

                        # Merge generated outputs back
                        batch = batch.union(gen_batch_output)
                        # Compute response_mask based on generated responses
                        batch.batch["response_mask"] = compute_response_mask(batch)

                    else: # OFFLINE Mode: Data is already loaded
                        # 'batch' already contains:
                        # batch.batch["input_ids"] (states)
                        # batch.batch["attention_mask"] (state attention mask)
                        # batch.batch["responses"] (actions from dataset)
                        # batch.batch["response_attention_mask"] (action attention mask, if needed)
                        # batch.batch["rewards"] (rewards from dataset - these will become token_level_scores)
                        # batch.batch["loss_mask"] (IMPORTANT for multi-turn, from dataset)
                        
                        # Ensure 'response_mask' is present, derived from loaded actions/loss_mask
                        # If 'loss_mask' is the definitive mask for rewards/advantages (especially for multi-turn)
                        # then 'response_mask' should align with it or be it.
                        if "loss_mask" in batch.batch:
                             # Assuming loss_mask from offline data is the primary mask for agent's actions
                            batch.batch["response_mask"] = batch.batch["loss_mask"]
                        elif "responses" in batch.batch and "attention_mask" in batch.batch: # Fallback if no specific loss_mask
                            # This fallback might not be ideal for multi-turn if 'responses' includes non-agent parts
                            # It's better if OfflineRLDataset provides a precise loss_mask / response_mask
                            batch.batch["response_mask"] = compute_response_mask(batch) # This uses full attention_mask's end
                        else:
                            raise ValueError("Offline batch must contain 'loss_mask' or 'responses' and 'attention_mask' to derive 'response_mask'.")

                        # UID for GRPO etc. - if your offline data has unique IDs per trajectory/step, load them.
                        if "uid" not in batch.non_tensor_batch:
                             batch.non_tensor_batch["uid"] = np.array([str(uuid.uuid4()) for _ in range(batch.batch.batch_size[0])], dtype=object)


                    # Balance batch (if enabled) - this logic can remain
                    if self.config.trainer.balance_batch:
                        self._balance_batch(batch, metrics=metrics)
                    ...
                    if self.use_reference_policy:
                        ...
                        with _timer("ref", timing_raw):
                            ref_log_prob = self.ref_policy_wg.compute_ref_log_prob(batch) # Computes log pi_ref(a_data|s_data)
                    ...
                    # Dumping generations might need adaptation for offline data (e.g., dump predictions of current policy on offline states)
                    rollout_data_dir = self.config.trainer.get("rollout_data_dir", None)
                    if self.is_offline_mode and rollout_data_dir:
                        # For offline, "rollout" means evaluating current policy on offline states
                        # This part would require generating sequences with the current actor
                        # on the `input_ids` from the offline batch, which is different from online rollout.
                        # Or, you might just log the offline data itself.
                        # For simplicity, we can skip this or log the loaded data.
                        pass
                    elif not self.is_offline_mode and rollout_data_dir: # Online mode dump
